...
/**
 * POST /api/redeem — Redeem winning conditional tokens for a resolved market.
 * Body: { conditionId: string, indexSet: number } (indexSet 1 = YES, 2 = NO).
 * Vercel serves this file at /api/redeem automatically.
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json({
      ok: true,
      message: 'Redeem API is reachable. POST with body: { conditionId: string, indexSet: 1 | 2 }',
    });
  }
